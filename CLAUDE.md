...
1. **Remote (claude.ai plan usage)** — `UsageService` hits claude.ai's undocumented internal `/api/organizations/{id}/usage` endpoint using a session cookie the user pastes in manually via `SettingsView`. The cookie is stored by `SessionStore` as an **AES-GCM encrypted file** under `~/Library/Application Support/ClaudeUsageMenuBar/session.enc` (perms `0600`), with the key derived from the Mac's hardware UUID — deliberately *not* the macOS Keychain, because an ad-hoc-signed app gets a fresh signature every rebuild and Keychain ACLs would then pop a scary "another app wants your saved item" prompt each time. The cookie is never logged or exposed through the local web server — only already-computed percentages/timestamps cross that boundary. `UsageStore.refresh()` polls this on a timer.
