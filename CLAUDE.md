...
**Menu bar icon + window (no `MenuBarExtra`)** — The app does *not* use SwiftUI's `MenuBarExtra`. `AppDelegate` (`@MainActor`) owns the single `UsageStore` instance and manually creates an `NSStatusItem`; clicking it calls `MainWindowPresenter.shared.toggle(store:)`, which opens `MenuContentView` in a real, freely-movable `NSWindow`/`NSWindowController` (not a popover anchored under the icon) and toggles it closed on a second click. `AppDelegate` re-renders the status item's image itself (via Combine, subscribing to `store.objectWillChange` merged with `store.activityMonitor.objectWillChange`) since there's no SwiftUI view doing that automatically anymore. `ClaudeUsageMenuBarApp`'s `body` is just an empty `Settings {}` scene — required only to satisfy SwiftUI's "an App needs at least one Scene" rule, otherwise unused.

**Menu bar icon rendering** — `MenuBarIconRenderer` builds a SwiftUI view (Claude logo + two progress bars + countdown text) and rasterizes it via `ImageRenderer`, then sets `isTemplate = false`, producing a plain `NSImage` assigned directly to `statusItem.button?.image`. This is required: `NSStatusItem` force-templates (monochrome-tints) any SwiftUI `Image`/`Shape` used directly, which is why a plain SwiftUI icon looks broken/invisible — going through `ImageRenderer` to a real bitmap is the only way to get color into the menu bar icon.

**Settings window** — `SettingsWindowPresenter` opens `SettingsView` in its own real `NSWindow`/`NSWindowController`, separate from `MainWindowPresenter`'s window, not a SwiftUI `.sheet()`. This follows the same pattern used for the main window and sidesteps ever having to reason about sheet/key-window interactions between the two.

**Swift concurrency conventions used throughout:**
- `UsageStore`, `ClaudeCodeActivityMonitor`, `ClaudeCodeHistoryStore`, `LocalWebServer`, `AppDelegate`, `MainWindowPresenter`, `SettingsWindowPresenter` are all `@MainActor` classes/final classes.
